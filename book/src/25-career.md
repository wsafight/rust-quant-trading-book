...
直接从第 24 章项目做差距测试。用两周实现最小闭环，哪项证据做不出来就回到对应章节。重点检查是否真正理解 Rust ownership/async cancellation，以及以前由团队其他组件承担的账务、保证金或运维边界。
