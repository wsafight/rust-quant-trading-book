...
> **学习导航**
>
> - 开始前：会使用基本命令行、编辑器，听说过 Git 即可。
> - 这一章学会：安装 Rust，并把一个小程序运行、测试和检查起来。
> - 大约需要：4–6 小时。
> - 做完留下：环境记录、第一个 Rust 项目和一份能力自评。

> **开章场景：第一天，不预测涨跌**
>
> 你刚接触量化交易，屏幕上只有两行报价：有人愿意用 100 元买，有人愿意用 102 元卖。带你入门的人没有让你预测下一秒的价格，只给了一个小任务：写程序读入这两个数字，算出中间价 101，并证明输入不合理时程序不会悄悄给出答案。
>
> 这个任务看起来很小，却包含了以后每天都会重复的工作：把现实规则写成代码，运行它，用测试检查边界，再让工具发现明显问题。**本章要解决的是：怎样搭好 Rust 环境，并完成第一个可以运行、可以检查的交易程序。**

> **第一次阅读建议**
>
> 第一次先完成 1.3 至 1.5：安装工具、运行中间价程序、执行测试。遇到暂时不懂的类型写法，可以先照着输入，确认每条命令有什么结果。程序跑通后再读 1.1、1.2 和 1.8，了解全书路线与真实工作；其余小节用于建立自己的学习记录。
...
表中有几个词会在全书反复出现，第一次阅读先知道大意即可：

- **固定样本（fixture）**：保存下来、每次测试都使用同一内容的一小份输入。
- **性能分析（profile）**：观察程序把时间和资源花在哪里，而不是靠猜测找慢点。
- **基于性质的测试（property test）**：让工具生成许多输入，反复检查某条规则始终成立。
- **时点一致（point-in-time）**：做历史判断时，只使用当时已经能够看到的信息。
- **监控面板（dashboard）与处置手册（runbook）**：前者展示系统状态，后者说明出问题时由谁做什么。

这些现在都不要求会实现。后续章节会在需要它们时重新解释，并给出具体例子。
...
这一章建立的不是交易策略，而是一套学习和交付方法：先把含糊需求拆成事实、状态、失败动作与证据，再用最小代码验证其中一个边界。`mid_half_ticks` 很小，却已经要求单位明确、非法盘口显式失败、半 tick 不丢失、极值不溢出。

离开本章前，试着不用正文回答下面三个问题：

- 为什么“行情超过 200 ms”还不是一个可实现的完整需求？
- 为什么测试通过只能证明指定输入下的行为，不能证明程序适合真实交易？
- 为什么 fixture、状态图和基准环境也属于工程交付？

当前示例仍把很多概念压在少量值里。下一章先学习 Rust 的表达式、控制流、数组与切片，把单个中间价扩展成一组可测试的交易判断；之后再逐步补上所有权和领域类型。
