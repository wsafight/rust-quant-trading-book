...
> **学习导航**
>
> - 开始前：理解第 3–4 章的借用、结构体和枚举。
> - 这一章学会：根据“怎样取数据”选择列表、队列或映射。
> - 大约需要：7–9 小时。
> - 做完留下：滚动窗口、有序价格档和不创建多余列表的统计代码。

> **开章场景：一条报价变成一整本订单簿**
>
> 程序现在不只处理最高买价，而要保存几十档买卖报价、最近一百笔成交和所有活动订单。最近成交要不断加入并淘汰旧记录，订单要按编号快速查找，买卖报价还要始终按价格排列。若把这些数据全塞进同一种列表，每次更新都会越来越麻烦。
>
> Rust 提供列表、双端队列、哈希映射和有序映射等集合。选择依据不是名字，而是数据会怎样加入、删除、查找和遍历。**本章要解决的是：面对一组持续变化的市场数据，怎样选择合适的容器并清楚地处理它们。**

> **第一次阅读建议**
>
> 先读 5.1 至 5.3：普通列表适合顺序数据，双端队列适合滚动窗口，映射适合按编号或价格查找。然后读 5.5 和 5.7，分别练习遍历数据与计算成交均价。排序、内存布局和性能选择第二次再读，第一次不需要背复杂度表。
...
迭代 `HashMap` 的顺序不稳定，不能把它直接用于需要每次一致的结果校验。生成校验值（checksum）前，需要先按稳定的 key 排序，或使用本身有序的结构。
...
在每条消息都会经过的高频路径上，反复 `format!`、`to_string()` 或拼接会申请新内存。先使用含义清楚、自己拥有数据的类型；只有性能分析证明这些分配确实重要时，再借用缓冲区或预留内存。
...
迭代器是惰性的：写下 `map` 或 `filter` 时通常还没有处理数据，直到 `sum`、`collect`、`for_each` 等最终操作才执行。连续的迭代器步骤通常可被编译器优化成循环，但中间使用 `collect` 会真实创建新集合。

## 5.7 计算成交量加权平均价

只算几笔成交价格的普通平均值，会让 1 个和 100 个的成交拥有相同影响。成交量加权平均价（VWAP）先把每笔价格乘以数量，再除以总数量。下面两笔成交是 `100 x 2` 和 `101 x 3`，所以结果是 `(200 + 303) / 5 = 100.6`。
...
权威成交均价通常保留精确分子和分母，或按交易所规则使用十进制数类型；示例的 `f64` 只用于展示结果。
...
先实现最简单正确的结构，并记录实际怎样读写数据，这就是工作负载（workload）。不要仅凭复杂度符号 Big-O 选择；数据有多少、通常访问哪些位置同样重要。
...
集合选择从 workload 出发：是否需要有序最优价、按 ID 查找、两端弹出、连续扫描或稳定回放。`BTreeMap` 并不因为名字中有 tree 就天然适合所有订单簿，`HashMap` 也不因为平均查找快就适合需要确定顺序的输出。先写出访问模式和数据规模，再用 benchmark 验证替换。

迭代器的价值也不只是代码更短。`iter`、`iter_mut` 与 `into_iter` 明确表达借用、独占修改和消费所有权；组合器可以避免无意的中间集合。但当错误处理、提前退出或状态更新被链式调用隐藏时，清楚的循环更好。

到这里，我们已经能批量处理数据，但尚未系统表达非法精度、溢出、缺失字段和可恢复失败。下一章将把类型、错误与测试合并成一道可执行边界。
