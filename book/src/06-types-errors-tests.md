...
>
> - 开始前：会使用枚举、集合和简单模块接口。
> - 这一章学会：阻止单位混用，让失败说明原因，并用测试守住边界。
> - 大约需要：8–10 小时。
> - 做完留下：ID/单位类型、小数转换、错误分类和算术测试。

> **开章场景：交易所发来一条不干净的数据**
>
> 程序收到一条行情：价格字段是 `"100.25"`，数量字段却是空字符串。最省事的写法会在解析失败时直接退出，或者偷偷把空值当成 0；前者让整条链路中断，后者会把错误数据伪装成正常报价。更麻烦的是，价格和数量若都用同一种数字类型，后续还可能被传反。
>
> 可靠程序要明确说明“哪里失败、为什么失败、调用方接下来能做什么”，并用测试守住小数、舍入、溢出和单位边界。**本章要解决的是：怎样用类型阻止混用，用错误信息保留原因，再用测试证明边界行为。**

> **第一次阅读建议**
>
> 先读 6.1 至 6.5，观察类型怎样阻止价格和数量传反，错误怎样告诉调用方下一步该重试、停止还是对账。再读 6.7 和 6.8，学会用一组输入覆盖正常值和边界值。小数转整数与基于性质的测试较长，可以在第一次完成基础错误处理后再读。

## 6.1 用新类型包装消除单位歧义

如果价格和数量都是 `i64`，编译器无法阻止参数传反。newtype 用不同结构体分别包装它们，让相同底层表示具有不同语义：
...
单元测试只检查一个较小功能。交易系统还需要其他层次的测试，因为“公式正确”“协议格式正确”和“故障后能够恢复”是不同问题：

| 测试层 | 目标 | 示例 |
| --- | --- | --- |
| 单元/表驱动 | 纯函数和产品组合 | 舍入、费用、多空方向盈亏 |
| 基于性质 | 对大量输入验证始终应成立的规则 | 累计成交不减少、订单簿不交叉 |
| 集成/契约 | 适配器与外部协议 | 快照/增量固定样本、签名 |
| 确定性回放 | 相同输入得到相同状态 | 意图、成交、仓位、盈亏 |
| 模糊测试 | 自动生成非法或极端输入 | 解码器、小数、状态机 |
...
这是教学版本：只接受非负普通十进制，并有一次字符串分配。生产中可以使用成熟的十进制数代码库或不分配新字符串的解析器，但要先固定含义和测试，再根据性能分析结果优化。

## 6.11 基于性质的测试从不变量出发

示例测试通常只覆盖我们想到的几个输入，基于性质的测试（property test）则让工具生成大量组合。关键不是“随机试一试”，而是先写出无论输入怎样都必须成立的规则。
...
本章把“调用者应该小心”改成了可执行契约：newtype 阻止不同单位混用，私有构造限制局部不变量，`Result` 保存失败上下文，测试从业务风险推导而不是从代码行数推导。

错误类型的真正用途是决定行动。非法输入应拒绝并保留证据，临时外部失败可能退避，订单请求超时则进入未知状态并触发查询；把它们都装进一个字符串，会让上层只能统一重试或退出。类似地，property test 证明的是生成范围内的不变量，不会自动补足协议 fixture 和跨组件故障测试。

下一章将为这些边界加入可替换实现。trait 会帮助 clock、decoder、fee model 和 adapter 在 live、replay 与测试之间替换；生命周期和 trait bound 则继续约束引用与线程边界。
