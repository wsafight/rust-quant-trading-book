...
交易系统处理的不是抽象时间序列，而是一组有规则、有队列、有费用并且由不同参与者共同产生的订单。市场微观结构研究价格如何在这些局部机制中形成，以及你的订单为什么成交或不成交。

> **学习导航**　前置：通过检查点二，能重建 valid L2 book｜目标：理解价差、队列、订单流、markout 与可检验机制｜预计：10–12 小时｜产出：imbalance/markout 研究、regime 分组和极端窗口回放
...
## 13.2 Mid、microprice 与 imbalance

mid 不考虑最优档数量。L1 imbalance 与 microprice 加入两侧数量：
...
maker 挂单提供流动性，taker 用可立即成交的订单消耗流动性。maker 可能获得更低费率，但承担选择性成交：
...
许多订单簿采用 price-time priority：更优价格先成交，同价更早订单在前。真实 venue 还可能有 pro-rata、特殊订单、隐藏数量、自成交保护或不同产品规则。
