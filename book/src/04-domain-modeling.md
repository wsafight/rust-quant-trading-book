# 第 4 章 Struct、Enum 与模式匹配：建立交易领域模型

基础类型只能表达“一个整数”，不能表达它是价格、数量还是序列。交易系统的正确性从领域模型开始：让合法状态容易构造，让危险组合难以出现。

> **学习导航**　前置：第 2–3 章，能读懂所有权错误｜目标：用 struct、enum、match 与私有字段表达领域不变量｜预计：6–8 小时｜产出：top-of-book、订单命令/事件和连接状态模型
...
两种 newtype 底层都是 `i64`，但函数签名阻止传反。字段暂时公开只是教学便利；生产类型可隐藏字段，通过构造函数保证正值、范围或 metadata 关联。
